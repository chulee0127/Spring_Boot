...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests()
				.antMatchers("/", "/login.do", "/loginFail").permitAll() // 누구나 접근 허용
				...
				.antMatchers("/crawling.do", "/sendEmail.do", "/nettySocket.do").hasAnyRole("admin")
				...
				.and() // 
				.exceptionHandling()
				.authenticationEntryPoint(new CustomAuthenticationEntryPoint()) // 인증되지 않은 사용자
				.accessDeniedHandler(new CustomAccessDeniedHandler()) // 인증은 되었으나 권한이 없는 사용자
		;
	}
